...
interface StandardCoffeesPageProps {
  onBack: () => void;
  items: Product[];
}

export function StandardCoffeesPage({ onBack, items }: StandardCoffeesPageProps) {
  useEffect(() => {
    window.scrollTo(0, 0);
  }, []);
    ...
    <div className="min-h-screen bg-gradient-to-b from-[#E6D3BA] to-[#C8A27A]">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="text-center">
            <div className="flex items-center justify-center gap-3 mb-4">
              <Coffee className="w-10 h-10 sm:w-12 sm:h-12 text-[#C8A27A]" strokeWidth={1.5} />
              <h1 className="text-3xl sm:text-4xl lg:text-5xl text-[#E6D3BA] tracking-tight">Standard Coffees</h1>
            </div>
            <p className="text-lg text-[#C8A27A] max-w-2xl mx-auto">
              Klasik kahve çeşitlerimiz ve özel tariflerimiz
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16 lg:py-20">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
          {/* coffees.map yerine items.map kullanıyoruz */}
